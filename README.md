...
这是第一阶段地面电脑程序：拉取无人机原生视频、被动接收 PX4/MAVLink 遥测、识别并锁定所选目标、计算跟随控制建议。视觉跟踪控制仍然**绝不向无人机发送**；另有默认关闭、必须由页面手动开启的地面 Offboard 零速度测试流、仅限空中的键盘 Offboard 速度发送器，以及以启动瞬间本地位置为高度零点的受保护 Offboard 起飞状态机。
...
2026-09-14：本地悬停至键盘改为单一轨迹、持续稳定检查和显式授权，参见
[平滑交接与当前阻断项](docs/KEYBOARD_HANDOFF.md)。当前仍缺实时详细光流融合/估计器重置数据接入：
新版本缺少重置证据会拒绝本地Offboard起飞，缺少融合证据会拒绝键盘交接；不得绕过，也未通过实飞验收。
本地起飞完成后使用新增的“授权键盘（平滑交接）”，不是旧键盘TX开关，不再自动倒计时开启。
...
- 网页程序允许受保护的飞控上行：手动开启的“地面 Offboard 心跳 / 零速度设定值”、仅限 `ARMED + IN_AIR + OFFBOARD` 的键盘速度设定值、每次均需现场确认的 ARM/DISARM、PX4 Pause、Land、RTL 单次命令，以及用户明确启动的 PX4 原生起飞或本地 Offboard 起飞状态机。它们均经本地路由入口 `127.0.0.1:14560` 转发给 `192.168.1.201:8080`；不发送 RC 覆盖或 `MANUAL_CONTROL`。视觉跟踪控制始终保持 TX=0。
...
- 视频画面下方提供键盘 Offboard 速度控制：`W/S`=机体系前后速度、`A/D`=机体系左右速度、左 `Shift/Alt`=上升/下降速度、`Q/E`=左/右偏航角速度。`Shift/Alt`不是油门或电机转速控制。真实 TX 只允许 `ARMED + IN_AIR + OFFBOARD`，并要求实体 Kill Switch 已解除且 CH8 物理模式开关仍在 Offboard；CH8 离开 Offboard 后发送器在下一周期自动关闭。严禁从地面用 Shift 起飞，必须先由实体遥控器起飞。松键或窗口失焦立即归零，浏览器输入中断超过 1.5 秒会自动停止发送。
...
- 本地 Offboard 起飞不需要全球坐标：程序把启动瞬间的 `LOCAL_POSITION_NED x0/y0/z0/yaw0` 作为定点与高度基准，以10 Hz预发送当前位置；只有页面提示就绪且飞手用实体CH8手动切入Offboard后，才发送一次正常ARM，再以0.30 m/s斜坡把目标设为 `z0 - 目标高度`。达到高度后仍持续发送最终定点，直到飞手切回Position或点击受控退出。CH6触发、CH8/模式退出、遥测过期会立即停止电脑设定值；姿态、漂移、超调或超时异常会请求LAND/DISARM。它不改PX4参数、不自动切模式、不发送RC覆盖；没有有效全球定位时RTL不可作为唯一安全手段。
...
7. 状态机完成并稳定悬停后，PX4保持其原生起飞/悬停模式，程序不会自动切换 Offboard。需要键盘控制时，必须由飞手用实体 CH8 切入 `OFFBOARD`，再打开键盘真实 TX。
